   ...
   <div
    ...
    >
      ...
      {expandedPostId === post.id ? (
        <>
          {post.location && <p className="extra-info">📍 {post.location}</p>}
          {post.caption && <p className="extra-info">📝 {post.caption}</p>}
          {post.hashtags && <p className="extra-info">🏷️ {post.hashtags}</p>}
          <div className="toggle-info-btn" onClick={() => setExpandedPostId(null)}>
            Show Less
          </div>
        </>
      ) : (
        // Only show "Show More" if there's any extra info to display
        (post.location || post.caption || post.hashtags) && (
        ...
        {showComments[post.id] && (
          <>
            {post.comments.length === 0 ? (
              ...
              // Added max-h-40 (max-height) and overflow-y-auto (scroll)
  ...
  return (
    <div className="dashboard-container">
      <div className="dashboard-header">
        ...
        {/* Profile Button next to Dark/Light Mode Toggle */}
        <button
          onClick={handleProfileClick}
          className="bg-transparent border border-cyan-500 text-cyan-600 hover:bg-cyan-50 hover:text-cyan-700 font-semibold px-4 py-2 rounded-full shadow-md transition flex items-center gap-2 mr-4" // Added mr-4 for spacing
        >
          View Profile
        </button>
